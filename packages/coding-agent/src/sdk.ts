...
async function createAgentSessionScoped(options: CreateAgentSessionOptions): Promise<CreateAgentSessionResult> {
	...
	try {
		...
		const toolSession: ToolSession = {
			...
			registerBackgroundableBashWait: toolCallId => session?.registerBackgroundableBashWait(toolCallId),
			unregisterBackgroundableBashWait: toolCallId => session?.unregisterBackgroundableBashWait(toolCallId),
